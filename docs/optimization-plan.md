...
2. **再改数据可靠性**: 缓存机制升级 (#8) + 多数据源备选 (#7)
3. **然后增强分析**: AI诊断 (#5) + 策略回测升级 (#4)
4. **最后锦上添花**: 通知系统 (#9) + 管理后台 (#10) + 其余低优项

### 关键决策点

| 议题 | 决策 |
|------|------|
| AI 模型选择 | 优先 DeepSeek-V3 (¥0.003/次)，GPT-4o 备选 ($0.015/次) |
| AI 防滥用 | 会员限额 + 积分扣除 + 同股同日缓存，三重保护 |
