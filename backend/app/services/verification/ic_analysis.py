...
def _lag(series: list, n: int) -> list:
    """返回 lag n 的序列（前 n 个为 None）。"""
    return [None] * n + series[:-n]


def _compute_factor(rows: list[dict], field: str) -> list[float | None]:
    """从一只股票的时序数据中计算单个因子。返回与 rows 等长的 list。"""
    n = len(rows)
    ...
    if field in ("pe", "pb", "total_mv", "turnover"):
        ...
        return [float(v) if v is not None and v > 0 else None for v in vals]
    if field == "pct_chg":
        return [r["pct_chg"] for r in rows]

    # 动量类
    if field == "chg3":
        return _momentum(closes, 3)
    if field == "chg5":
        return _momentum(closes, 5)
    if field == "chg10":
        return _momentum(closes, 10)
    ...
    if field == "vol_ratio":
        ...
        for i in range(n):
            if ma20[i] and ma20[i] > 0:
                out[i] = volumes[i] / ma20[i]
        return out

    # 距离低点（20日最低价）
    if field == "dist_from_low":
        return _dist_from_low(rows, 20)
    ...
    if field == "ma_slope":
        ...
        out = [None] * n
        for i in range(2, n):
            if ma10[i] is not None and ma10[i - 2] is not None and ma10[i - 2] > 0:
                out[i] = (ma10[i] - ma10[i - 2]) / ma10[i - 2] * 100
        return out

    # 回撤（20日最高价回撤）
    if field == "drawdown":
        return _drawdown(rows, 20)

    # 近期3日涨幅
    if field == "chg3_recent":
        return _momentum(closes, 3)

    logger.warning(f"Unknown factor field: {field}")
    return [None] * n


def _momentum(closes: list[float], n: int) -> list[float | None]:
    out = [None] * len(closes)
...
def _dist_from_low(rows: list[dict], n: int) -> list[float | None]:
    out = [None] * len(rows)
    for i in range(n - 1, len(rows)):
        lows = [rows[j]["close"] for j in range(i - n + 1, i + 1)]
        min_low = min(lows)
        if min_low > 0:
            out[i] = (rows[i]["close"] - min_low) / min_low * 100
    return out


def _drawdown(rows: list[dict], n: int) -> list[float | None]:
    out = [None] * len(rows)
    for i in range(n - 1, len(rows)):
        highs = [rows[j]["close"] for j in range(i - n + 1, i + 1)]
        peak = max(highs)
        if peak > 0:
            out[i] = (peak - rows[i]["close"]) / peak * 100
    return out
